...
#+end_src
   ...
   Foi o vetor de pixels que representam a imagem, o que naturalmente
   sugere a utilização de uma *matriz NxN*, foi salvo utilizando um
   vetor de *uma dimensão* salvando as linhas em sequência (/row-major
   scheme/) de modo que uma imagem com *largura N*, o pixel(i,j) seja
   salvo no vetor na posição Array[i * N + j].
...
**** Solução
    ...
    - Os outros processos tiveram o trabalho de ler o arquivo com
      *2L * X * 3* pixels, e somar com o vetor que eles haviam
      calculado. Após feito esse cálculo, o arquivo foi reescrito
